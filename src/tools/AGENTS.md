...
- Preserve concurrency semantics in scheduling, caching, retry, and rate-limit helpers. Callers
  ...
  success and failure.
- `shared/tools/fetchRequest.ts` is the common network boundary. Maintain its normalized response
  shape and `RequestError`/`HTTPError`/`TimeoutError` distinctions; redact sensitive response/request
  data in errors and debug output.
