...
- MariaDB is the persistent store. Sequelize model definitions and schema synchronization live in
  `src/db.ts`; there is no separate migration framework.
...
- For schema changes, review model types, `Model.init`, indexes/associations, all query methods, and
  compatibility with existing MariaDB data. `sequelize.sync()` is not a substitute for planning a
  destructive production schema transition.
