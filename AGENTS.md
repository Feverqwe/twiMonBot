...
- MariaDB is the persistent store. Sequelize models live in `src/db/`; schema changes are applied
...
- `twiMonBot` and the adjacent `../ytWatchBot` checkout are sibling projects maintained together.
  `twiMonBot` owns multi-provider livestream monitoring behavior; `ytWatchBot` owns YouTube video
  discovery/delivery behavior.
...
- Verify sibling infrastructure: `npm run shared:check`.
